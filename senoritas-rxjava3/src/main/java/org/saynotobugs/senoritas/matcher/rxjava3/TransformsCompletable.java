...
import org.saynotobugs.senoritas.matcher.rxjava3.adapters.CompletableSubjectAdapter;
...
public final class TransformsCompletable<Up, Down> implements Matcher<Function<? super TestScheduler, ? extends CompletableTransformer>>
{
    ...
    public Verdict match(Function<? super TestScheduler, ? extends CompletableTransformer> actual)
    {
        ...
        return new AllOfFailingFast<>(
            new Expanded<>(e -> e.matchers(t, new CompletableSubjectAdapter<>(upstream)), mEvents)
        ).match(testAdapter);
    }
